...
#define fs 46875
#define sin_increment 0.00013404128
...
void record(){
    ...
    writeToSRAM(counter, (int)sampleInL);
    //write right sample into next address
    counter=(counter+1)&BUF_MASK;
    writeToSRAM(counter, (int)sampleInR);
    //done writing sample (SRAM full)
    if(counter==0x3FFFF){
        for(Uint32 i=0; i<=0x3FFFF; i++){
            float32 sin_val=sin((i/2)%fs*2*3.14/46875);
            Uint32 delay=((int32)((sin_val+1)/2*0.015*fs));
            float32 curr=(int)readFromSRAM(i);
            float32 delayed=(int)readFromSRAM((i-delay)&BUF_MASK);
            curr=0.5*curr+0.5*delayed;
            writeToSRAM(i, (int)curr);
            i++;
             curr=(int)readFromSRAM(i);
         delayed=(int)readFromSRAM((i-delay)&BUF_MASK);
        curr=0.5*curr+0.5*delayed;
            writeToSRAM(i, (int)curr);
        }
        //finish recording
