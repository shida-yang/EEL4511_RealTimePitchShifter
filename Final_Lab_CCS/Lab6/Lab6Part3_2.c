 ...
 * Name:        Shida Yang
 * Lab #:       23195
 * Description: This program samples sound at 32kHz, interpolate by 2, and play
 *              it back at different rates based on DIP switches
 */
...
void interpolate(){
    //read the first data
    float32 first=(int)readFromSRAM(0);
    float32 second;
    //loop until the end of SRAM
    for(Uint32 i=2; i<=0x3FFFE; i+=2){
        //read the second data
        second=(int)readFromSRAM(i);
        //interpolate
        float32 avg=0.5*(first+second);
        //store the interpolated data into SRAM
        //between first and second data
        writeToSRAM(i-1, (int)avg);
        //the second data in this iteration is the
        //first data in the next iteration
        first=second;
    }
}
